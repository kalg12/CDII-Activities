    ...
    chartReadyPromise = new Promise((resolve, reject) => {
        const tryLoad = (index) => {
            if (index >= cdnUrls.length) {
                reject(new Error('No se pudo cargar Chart.js desde los CDNs configurados'));
                return;
            }
...
function initializeCharts() {
    ...
    if (typeof Chart === 'undefined') {
        ...
        showFeedback('Error: Chart.js no se pudo cargar. Recarga la página.', 'error');
        return;
    }
    ...
    console.log('Botones de gráficas encontrados:', { barBtn, pieBtn, updateBtn });
...
function generateBarChart() {
    ...
    if (typeof Chart === 'undefined') {
        showFeedback('Chart.js no está disponible. Verifica tu conexión y recarga.', 'error');
        return;
    }
...
function updateCharts() {
    ...
    if (!AppState.charts.bar && !AppState.charts.pie) {
        showFeedback('No hay gráficas para actualizar. Genera una primero.', 'error');
    }
}
...
function handleCompletion() {
    ...
    const wordCount = interpretation.split(/\s+/).filter(word => word.length > 0).length;
    
    // Validaciones
    if (AppState.tableData.length < 4) {
        showFeedback('Necesitas al menos 4 filas para completar la actividad', 'error');
        return;
    }
    ...
    if (wordCount < 20 || wordCount > 80) {
        showFeedback('Tu interpretación debe tener entre 20 y 80 palabras', 'error');
        document.getElementById('interpretation').focus();
        return;
    }
...
function generateCompletionCode() {
    ...
    const hash = btoa(AppState.studentData.fullName + AppState.studentData.career + AppState.studentData.group)
        .replace(/[^a-zA-Z0-9]/g, '')
        .substring(0, 8)
        .toUpperCase();
...
function showFinalResult() {
    ...
    document.getElementById('summaryName').textContent = AppState.studentData.fullName;
    document.getElementById('summaryCareer').textContent = AppState.studentData.career;
    document.getElementById('summaryGroup').textContent = AppState.studentData.group;
    document.getElementById('summaryDate').textContent = new Date().toLocaleString('es-MX');
    ...
    document.getElementById('chartTypeGenerated').textContent = chartTypes.join(' y ') || 'Ninguna';
    
    // Actualizar interpretación
    document.getElementById('finalInterpretation').textContent = AppState.interpretation;
    
    // Actualizar código
    document.getElementById('completionCode').textContent = AppState.completionCode;
...
function copyToForum() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(forumText)
            .then(() => {
                showFeedback('Texto copiado. Pégalo en el foro: "Modelación de datos"', 'success');
            })
            .catch(err => {
...
function exportToPDF() {
    showFeedback('Usa Ctrl+P o Imprimir → Guardar como PDF para exportar', 'info');
    window.print();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadChartJs()
        ...
        .catch(error => {
            ...
            showFeedback('Error: Chart.js no se pudo cargar. Verifica tu conexión y recarga.', 'error');
